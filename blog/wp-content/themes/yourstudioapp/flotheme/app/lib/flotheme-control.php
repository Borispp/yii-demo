...
class Flotheme_Control
{
	...
	public function render($field, $config = null, $value = null, $fieldPrefix = 'flotheme')
	{
	...
	protected function _renderText()
	{
		...
		$html .= '<input type="text" class="text" name="'.$this->_fieldPrefix.'[' . $this->_field . ']" id="flothemes_option_' . $this->_field . '" value="' . $value . '" />';
		$html .= '</div>';
	...
	protected function _renderImage()
	{
		...
		if ($this->_value) {
			...
			$html .= '<input type="file" title="' . $this->_field . '" name="'.$this->_fieldPrefix.'[' . $this->_field . ']" id="flothemes_option_' . $this->_field . '" />';
		}
		$html .= '</div>';
	...
	protected function _renderTextarea()
	{
		...
		$html = '<div class="value">';
		$html .= '<textarea name="'.$this->_fieldPrefix.'[' . $this->_field . ']" id="flothemes_option_' . $this->_field . '" cols="40" rows="5">' . $value . '</textarea>';
		$html .= '</div>';
		$html .= '<div class="description">'. $this->_config['description'] .'</div>';
	...
	protected function _renderCheckbox()
	{
		...
		$html = '<div class="value">';
		$html .= '<select name="'.$this->_fieldPrefix.'[' . $this->_field . ']" id="flothemes_option_' . $this->_field . '">';
		$html .= '<option value="0">No</option>';
		$html .= '<option value="1" ' . ($value ? 'selected="selected"' : '') . '>Yes</option>';
		$html .= '</select>';
		//$html .= '<input type="checkbox" name="'.$this->_fieldPrefix.'[' . $this->_field . ']" id="flothemes_option_' . $this->_field . '" value="1" ' . ($value ? 'checked="checked"' : '') . ' />';
		$html .= '</div>';
		if (array_key_exists('description', $this->_config))
			$html .= '<div class="description">'. $this->_config['description'] .'</div>';
		return $html;
	...
	protected function _renderSelectbox()
	{
		...
		$html = '<div class="value">';
		$html .= '<select name="'.$this->_fieldPrefix.'[' . $this->_field . ']'.($isMultiple ? '[]' : '').'" id="flothemes_option_' . $this->_field . '"'.($isMultiple ? ' multiple="multiple" class="multiple-select"' : '').'>';
