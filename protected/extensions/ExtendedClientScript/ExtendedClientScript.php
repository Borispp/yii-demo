...
class ExtendedClientScript extends CClientScript
{
	...
	*/
	...
	public $forceJsFiles = array();
	public $forceCssFiles = array();
	...
	public function renderHead(&$output)
	{
		...
		if ($this->combineCss)
		{
			if (count($this->cssFiles) !== 0)
			{
				...
				{
					foreach($this->forceCssFiles as $cssFileMask)
					{
						if (preg_match('~^'.$cssFileMask.'.*~', $url) && !$this->isRemoteFile($url))
						{
	...
	private function renderJs($output, $pos)
	{
		if ($this->combineJs)
		{
			if (isset($this->scriptFiles[$pos]) && count($this->scriptFiles[$pos]) !==  0)
			{
				...
				{
					foreach ($this->forceJsFiles as $forceFile)
						if (!preg_match('~^'.$forceFile.'.*~', $fileName))
							$fileName = false;
				}
