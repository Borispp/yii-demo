<li id="album-photo-<?php echo $entry->id?>" <?php echo $entry->isCover() ? 'class="cover"' : ''; ?>>
	<figure data-src="<?php echo $entry->url();?>">
		...
		<span class="menu">
			<?php echo YsaHtml::link('View', array('photo/view/' . $entry->id), array('class' => 'view icon i_aperture', 'title' => 'View Photo')); ?>
			&nbsp;|&nbsp;
			<?php echo YsaHtml::link('Set Cover', '#', array('class' => 'setcover icon i_star', 'title' => 'Make Album Cover', 'rel' => $entry->id)); ?>
			&nbsp;|&nbsp;
			<?php echo YsaHtml::link('Delete', array('photo/delete/' . $entry->id), array('class' => 'del icon i_x_alt', 'title' => 'Delete Photo')); ?>
		</span>
