...
/**
 * This is the model class for table "ipad_notification".
 *
 * The followings are the available columns in table 'ipad_notification':
 * @property string $id
 ...
 */
class IpadNotification extends YsaActiveRecord
{
	/**
	...
	public function relations()
	{
		...
		return array(
			...
			'event'			=> array(self::MANY_MANY, 'Event', 'ipad_notification_event(app_notification_id, event_id)'),
			'client'		=> array(self::MANY_MANY, 'Client', 'ipad_notification_client(app_notification_id, client_id)'),
	...
	/**
	 * Marks notification as sent (Creates ipad_notification_state record with link to device id)
	 * @param $deviceId
	 * @return void
	 */
	...
	public function findByClient(Client $obClient, $deviceId = NULL)
	{
		...
		$criteria->join = 'LEFT JOIN ipad_notification_client ON app_n.id = ipad_notification_client.app_notification_id'.
		' LEFT JOIN ipad_notification_event ON ipad_notification_event.app_notification_id = app_n.id'.
		' LEFT JOIN client_events ON client_events.event_id = ipad_notification_event.event_id';
		$criteria->params = array(':clientId' => $obClient->id);
		$criteria->condition = 'client_events.client_id = :clientId OR ipad_notification_client.client_id = :clientId';
		if ($deviceId)
		{
			$criteria->params[':deviceId'] = $deviceId;
			$criteria->condition = '('.$criteria->condition.') AND NOT (SELECT COUNT(*) FROM ipad_notification_state WHERE ipad_notification_state.app_notification_id = app_n.id AND device_id = :deviceId)';
		}
